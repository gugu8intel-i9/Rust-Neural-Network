...
impl ModelDashboard {
    ...
    pub fn render(&self) -> String {
        ...
        for (i, (name, inp, out)) in self.layers.iter().enumerate() {
            let x = gap + i * (bw + gap);
            let y = (sh - bh) / 2;
            let col = if name.starts_with("Linear") {
                "#4A90D9"
            ...
            if i > 0 {
                let px = gap + (i - 1) * (bw + gap) + bw;
                let _=write!(svg,"<line x1=\"{}\" y1=\"{}\" x2=\"{}\" y2=\"{}\" stroke=\"#555\" stroke-width=\"2\"/>",px,y+bh/2,x,y+bh/2);
            }
            ...
            if *inp > 0 {
                let _=write!(svg,"<text x=\"{}\" y=\"{}\" text-anchor=\"middle\" fill=\"#ddd\" font-size=\"11\">{} to {}</text>",x+bw/2,y+42,inp,out);
            }
        }
        ...
        let _ = write!(
            h,
            "{}</div><div class=\"stat-label\">Layers</div></div>",
            self.layers.len()
        );
...
impl TrainingDashboard {
    ...
    pub fn add_point(&mut self, epoch: usize, loss: f64) {
    ...
    pub fn render(&self) -> String {
        ...
        if p.is_empty() {
            return format!(
                "<html><body><h1>{}</h1><p>No data.</p></body></html>",
                self.title
            );
        }
        ...
        let path: String = p
            .iter()
            .map(|(e, l)| {
                let x = 40.0 + (*e as f64 / me as f64) * 520.0;
                let y = 40.0 + (1.0 - (l - mn) / r) * 220.0;
                format!("{:.1},{:.1}", x, y)
            })
            .collect::<Vec<_>>()
            .join(" ");
        ...
        let _ = write!(
            h,
            "<div class=\"stat\"><div class=\"sv\">{:.4}</div><div class=\"sl\">Final</div></div>",
            fl
        );
        ...
        let _ = write!(
            h,
            "<polyline points=\"{}\" fill=\"none\" stroke=\"#e94560\" stroke-width=\"2.5\"/>",
            path
        );
        let _ = write!(
            h,
            "<text x=\"30\" y=\"35\" fill=\"#888\" font-size=\"11\">{:.3}</text>",
            ml
        );
        let _ = write!(
            h,
            "<text x=\"30\" y=\"275\" fill=\"#888\" font-size=\"11\">{:.3}</text>",
            mn
        );
        let _ = write!(
            h,
            "<text x=\"540\" y=\"278\" fill=\"#888\" font-size=\"11\">Epoch {}</text>",
            me
        );
...
pub fn tensor_heatmap_html(tensor: &crate::tensor::Tensor, max_display: usize) -> String {
    ...
    for (i, &v) in data.iter().enumerate() {
        ...
        if (i + 1) % cols == 0 && i + 1 < data.len() {
            cells.push_str("</tr><tr>");
        }
    }
    format!(
        "<table style=\"border-collapse:collapse;font-family:monospace\"><tr>{}</tr></table>",
        cells
    )
}
...
mod tests {
    ...
    #[test]
    fn model_html() {
        let d = ModelDashboard::new("MLP")
            .layer("Linear", 10, 20, "he")
            .layer("ReLU", 20, 20, "")
            .layer("Linear", 20, 5, "he");
        let h = d.render();
    ...
    fn full_dash() {
        ...
        let mut t = TrainingDashboard::new("T");
        t.add_point(1, 1.0);
