...
use rust_nn::diffusion::{NoiseSchedule, ScheduleType, DDPM};
...
fn mamba_block_preserves_shape() {
    ...
    assert_eq!(
        y.shape(),
        vec![2, 5, 8],
        "MambaBlock must preserve [batch, seq, d_model]"
    );
}
...
fn mamba_block_is_differentiable() {
    ...
    assert!(
        ...
        "MambaBlock gradients should be non-zero"
    );
}
...
fn mamba_full_has_parameters() {
    ...
    assert!(
        params.len() > 10,
        "Mamba should expose many parameter tensors, got {}",
        params.len()
    );
}
...
fn mamba_block_train_step_reduces_loss() {
    ...
    assert!(
        last < first,
        "Mamba loss should decrease ({last:.4} >= {first:.4})"
    );
}
...
fn noise_schedule_linear_is_valid() {
    ...
    assert!(
        (s.alpha_cumprod[0] - 1.0).abs() < 1e-3,
        "alpha_cumprod[0] should be ~1"
    );
}
...
fn q_sample_endpoints() {
    ...
    assert!(
        (d0[0] - 1.0).abs() < 0.05,
        "q_sample at t=0 should be ~x_0, got {}",
        d0[0]
    );
    ...
    assert!(
        d99[0].abs() < 1.0,
        "q_sample at high t with zero noise should be small, got {}",
        d99[0]
    );
}
...
fn ddpm_train_batch_runs_and_reduces_loss() {
    ...
    assert!(
        last < first,
        "DDPM loss should decrease ({last:.4} >= {first:.4})"
    );
}
...
fn ddpm_sample_shape() {
    ...
    assert!(
        samples.data().iter().all(|v| v.is_finite()),
